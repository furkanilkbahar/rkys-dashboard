...
import type { AdminTicketThread } from "@/lib/data/supportTickets";
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        <AdminPageHeader title={thread.subject} />
      ...
      <div className="flex flex-col gap-3">
        {thread.messages.map((message) => (
          <div
            key={message.id}
            className={`max-w-lg rounded-md border border-border p-3 text-sm ${message.sender === "platform" ? "self-start bg-accent/30" : "self-end"}`}
          >
