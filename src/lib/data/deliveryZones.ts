...
export async function getAdminDeliveryZones(tenantId: string): Promise<AdminDeliveryZone[]> {
...
export async function getActiveDeliveryZones(tenantId: string): Promise<DeliveryZoneOption[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("delivery_zones")
