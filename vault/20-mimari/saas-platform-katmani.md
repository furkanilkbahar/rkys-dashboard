...
- **Abonelik kapisi (D101):** trial'in BITISI artik bir sonuc doguruyor. `resolve_tenant_by_domain` `subscription_active` dondurur, proxy tenant'in TUM yuzeylerini (misafir QR menusu dahil) `/abonelik-gerekli`'ye cevirir. Muaf: `/admin/login`, `/admin/billing`, `/api/webhooks` — odeyip geri donus yolu acik kalmali. **Pasiflik saklanmiyor, turetiliyor**: `is_subscription_active()` her istekte hesaplanir, zamanlanmis gorev yok, odeme yapildigi an servis geri gelir. Havale/EFT icin `mark_subscription_paid` (platform admin only, `provider='manual'`).
