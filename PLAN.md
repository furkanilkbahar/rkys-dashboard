...
- [x] `admin/ingredients-recipe.spec.ts` (S34, yalnızca mobile-safari) — **kök neden bulundu ve kapandı (2026-09-02, `ac481dc`).** Teşhis "malzeme listede geç görünüyor" değildi: `recipe-editor.tsx:71` kaydetmenin ardından `router.refresh()` çağırıyor, "Kaydedildi." o yenileme BİTMEDEN görünüyor ve testin bir sonraki `page.goto`'su yarı yolda kesiliyordu (`Navigation to /cashier/order is interrupted by another navigation to /admin/ingredients/recipes/<id>`). `networkidle` beklemesi bunu kapatmadı — yenileme ağ boşaldıktan SONRA başlayabiliyor. Çözüm `helpers/tenant.ts` içindeki `gotoSettled` (gezinme yeniden denenir; `goto` yan etkisiz). Artık iki tarayıcıda da geçiyor.

**Faz 19'a 2026-09-02'de eklenen bulgular (CI ilk kez gerçekten koştuğu için görünür oldu):**
- [x] **E2E projeleri paralel koşuyordu ve birbirini bozuyordu** (`9ff546c`). chromium-desktop ile mobile-safari aynı spec'leri AYNI ANDA, paylaşılan acme tenant'ına karşı koşuyordu; tenant durumu değiştiren her spec kendi ikiziyle yarışıyordu. Lokalde birebir üretildi: `theme-switch` tek başına iki projede koşturulduğunda chromium'un `finally`'si temayı "kagit"a alırken mobile-safari "kor" bekliyor. CI'da projeler artık sırayla koşuyor — 18 hata → 6, chromium tarafı tamamen yeşil.
- [x] **WebKit hidrasyon yarışı** (`ac481dc`). `waiter-pin-login` sayfa yüklenir yüklenmez formu dolduruyordu; WebKit sonra hidrate olunca react-hook-form alanı devralıp boşaltıyordu (snapshot'ta "Ad Soyad" boş, sonrakiler dolu). `networkidle` `fill`'i düzeltti ama tıklamayı düzeltmedi — ağı ölçüyor, React'in dinleyici bağlamasını değil. Rol seçicisini (saf client bileşeni) açmak hidrasyonu kanıtlıyor.
- [x] **Garson panelinde tıklama yutulması** (`4a2dda9`). Panel realtime + D30 5sn polling ile sürekli render oluyor; mousedown/mouseup arasına denk gelen render tıklamayı yutuyor. Hata anındaki snapshot butonu `[active]`, durumu "Bağlı" gösteriyordu ama sayfa hâlâ /waiter'dı. Çıkış idempotent olduğu için tıklama tekrarlanıyor. **Ürün kodu suçsuz çıktı:** `router.push`+`refresh` denenip geri alındı, sebep o değildi.
- [ ] **Kalan iki mobile-safari hatası** (CI koşumu 33567634327, `4a2dda9`): `multi-language-currency` (S16) ve `waiter-call-realtime`. Aynı koşumda chromium **exit=0** (96 geçti, 5 flaky), mobile-safari 88 geçti / 2 hata / 10 flaky. Toplam gidişat: 18 hata → 6 → 2. Flaky küme (proje İÇİ çakışma; `menu-crud:87`, `report-schedules` ×2, `reservations`, `staff-performance-goals`, `webhooks`, `fiscal-integration`, `gift-cards`, `module-addon-pricing`, `plan-table-limit`) retry'da geçiyor — kalıcı çözümü tek kullanımlık tenant deseni (bkz. D106'da elenen ikinci seçenek).
