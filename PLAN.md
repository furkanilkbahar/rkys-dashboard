...
- [x] **Adım 3 — `(waiter)` + `(kitchen)` + `(cashier)` + `(courier)`** *(2026-08-03 tamamlandı)*
      ...
      **Test notu:** `chromium-desktop`'ta staff+cashier paketi 17 geçti / 0 kırık (3 flaky, retry'de yeşil). Kırık görünen her senaryo `git stash` ile HEAD'de de doğrulandı — `kitchen-station-filter` HEAD'de 3/5, bu Adım'la 1/5 kırık; `comps-refunds` HEAD'de 3/3 kırık; `pay` HEAD'de 2/3 kırık; `waiter-pin-login` (mobile-safari) HEAD'de 3/3 kırık. Hiçbiri bu Adım'ın regresyonu değil.

      **Faz 21 takip maddeleri (kapsam dışı, ayrı ele alınacak):**
      1. **Yerel demo verisi kirli** — 27 tenant'ın 24'ü `test-*`, acme'de 5 kategori (seed 2), 40 masa, 182 sipariş. Bu yüzden `schema-and-seed` ve 5 RLS testinin **sayı** assertion'ı kırık (izolasyon assertion'ları geçiyor, RLS sağlam). `supabase db reset` gerekiyor — veri kaybı olduğu için kullanıcı onayı bekliyor. Aynı kirlilik Adım 2'nin gerçek ekran görüntüsü maddesini de bloke ediyor.
      2. **E2E hidrasyon yarışı** — `mobile-safari`'de birçok senaryo, react-hook-form formuna hidrasyondan önce yazıp/tıklayarak kırılıyor (`kitchen-station-filter`, `waiter-pin-login`, `pay`, `comps-refunds`). Testlerin davranış sözleşmesi değil, bekleme stratejisi sorunu; Faz 19 bakım borcu listesine ait.
      3. `themes` tablosunun asimetrik GRANT'i (0052): `service_role` yazabiliyor ama okuyamıyor.
