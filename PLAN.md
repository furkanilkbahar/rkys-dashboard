...
- [x] **Adım 5 — Kayıtta ödeme kaldırıldı, trial'ın bitişi kapıya bağlandı (D101).** Açık madde 2'nin ("Demo varsayılan seçili") kullanıcıya dönen yüzü şuydu: "Ücretsiz Deneyin"e basan ziyaretçi ₺0'lık Demo planıyla checkout sayfasına düşüyordu. Kök sebep tek bir hata değil, **D18 ile D80'in çakışmasıydı** — kayıt akışı ödeme istiyor, ana sayfa "14 gün kartsız deneme" vaat ediyordu. Kullanıcı D18 yönünü seçti. Kayıt artık ödeme almıyor; ödeme `/admin/billing`'e taşındı; trial dolduğunda proxy tenant'ı `/abonelik-gerekli`'ye çeviriyor (giriş + ödeme yolu açık kalıyor); süper admin havale/EFT tahsilatını "Ödemesi alındı" ile işaretleyip tenant'ı pasiflikten çıkarabiliyor. **Pasiflik türetiliyor, saklanmıyor** — zamanlanmış görev yok, ödeme yapıldığı an servis geri geliyor. Varsayılan plan seçimi `defaultSelectablePlanId`'ye çıkarıldı (aynı `plans[0]` hatası `/admin/billing` seçicisinde de vardı). Migration 0093.

      **Faz 24 açık maddeleri:**
      1. **Migration 0092 üretim Supabase'ine hâlâ uygulanmadı** — kullanıcı onayı bekliyor. Uygulanana kadar üretimde tüm planlar (Demo dahil) görünür; geri düşüş sayesinde tablo boş değil. Uygulandıktan sonra üretimdeki Demo planının `key`'i `demo` değilse Süper Admin'den "Ana sayfada göster" anahtarıyla kapatılır. **Migration 0093 de aynı kuyrukta ve sırası D99'un kalıcı kuralı gereği kritik:** 0093 uygulanmadan uygulama kodu üretime çıkarsa `resolve_tenant_by_domain` eski dönüş tipiyle kalır, `subscription_active` `undefined` gelir ve abonelik kapısı **her tenant'ı kapatır**. Bu faz, 0093 üretime uygulanmadan production'a çıkarılmamalı.
