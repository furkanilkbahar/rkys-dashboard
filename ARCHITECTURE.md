...
> Mimari revizyon (D87, bug-hunt 2026-08-01): garson paneli (`/waiter`) artık owner'ın admin oturumundan **bağımsız** bir ikinci Supabase Auth oturumu destekliyor — aynı tarayıcıda, farklı bir cookie adı altında (`sb-waiter-auth-token`, `lib/supabase/waiterClient.ts`). Garson `/waiter/login`'de PIN girer (mevcut vardiya cihaz+PIN altyapısı, `verify_staff_pin_identity` RPC'siyle yeniden kullanılır), sunucu o personelin GERÇEK hesabı için `auth.admin.generateLink`+`verifyOtp` ("parolasız giriş") ile ikinci cookie'de oturum açar. `lib/supabase/server.ts`'in `createClient()`'ı iki cookie'den hangisinde geçerli bir personel oturumu varsa onu döner — `/waiter` altında (yalnızca orada, `proxy.ts`'in enjekte ettiği bir header'la) garson cookie'si önceliklidir, diğer TÜM yüzeylerde davranış değişmez. Bu, RLS politikalarına veya herhangi bir RPC'ye hiç dokunmadan iki bağımsız "kim giriş yapmış" durumunu tek tarayıcıda taşımanın yolu.
...
- **Demo menü şablonları (D85):** Onboarding'in "Demo veriyle keşfet" ve "Sıfırdan kur → Şablon Menü" adımları artık tek sabit şablon yerine `lib/onboarding/menuTemplates.ts`'teki 3 statik şablondan (kafe/restoran/pastane, her biri 4 kategori × 4 ürün) birini sunar — kod içi TR/EN içerik `content_translations`'a aynı mevcut desenle yazılır. Görseller ayrı bir kod yolu değil, mevcut `menu-images` Storage bucket'ının tenant-prefixsiz paylaşılan `_templates/<key>/<slug>.webp` alt yolunda tutulur (public-read policy zaten bucket geneli, RLS yazma kısıtı yalnızca `insert/update/delete`'i etkiliyor — `applyMenuTemplate` server action'ı `getPublicUrl` ile aynı diğer görsel yükleme akışlarındaki gibi URL üretir).
