...
- `tests/e2e/staff/waiter-call-realtime.spec.ts`: gerçek Realtime (WebSocket) teslimatına bağlı olduğu için ara sıra zaman aşımına uğrayabiliyor (playwright.config.ts'teki `retries: 2` yorumu bunu zaten öngörüyor) — izole çalıştırıldığında güvenilir şekilde geçiyor. Ayrıca bu test kendi açtığı "Masa 3" çağrısını `finally`'de temizlemiyor; aynı gün içinde çok sayıda tam E2E koşumu (ör. bir faz kapanışında art arda birkaç kez) paylaşılan "acme" tenant'ında karşılanmamış eski çağrı satırları biriktirebilir, bu da testin `.first()` locator'ının eski bir satırla eşleşip yanlışlıkla başarısız görünmesine yol açabilir (Faz 10 kapanışında gözlemlendi, 2026-07-24 — kök neden test kodu değil, veri temizliği eksikliği). Geçici çözüm: `delete from waiter_calls where status = 'open'` ile temizlenebilir; kalıcı çözüm (testin kendi çağrısını `finally`'de acknowledge/silmesi) ayrı bir görevde ele alınacak.
- `tests/integration/orders/webhooks.integration.test.ts` (gerçek ağ testleri, "Webhook teslimatı: imzalama + retry"): Faz 11 Adım 2 kapanışında (2026-07-24) art arda birkaç kez başarısız oldu — `net.http_post` manuel olarak psql'den test edildiğinde ~3sn içinde 200 döndüğü doğrulandı (pg_net/cron altyapısı sağlıklı), ama test yalnızca 2 saniye bekleyip `reconcile_webhook_deliveries` çağırıyor. Kısa süre içinde aynı httpbin.org uçlarına yapılan tekrarlı (debug amaçlı elle + otomatik) çok sayıda istek muhtemelen geçici yavaşlama/rate-limit'e yol açtı. RPC/kod tarafında bir sorun yok; testin sabit 2sn bekleme süresi dış servisin gecikme varyansına karşı kırılgan — ayrı bir görevde artırılabilir (ör. polling ile bekleme).
